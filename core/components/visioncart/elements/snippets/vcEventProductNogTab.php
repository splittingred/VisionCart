...
switch($scriptProperties['vcAction']) {
	case 'getParams':
		...
		$tabParams['active'] = false; // Active false
		$tabParams['showTab'] = true; // true for a GUI, false for just a plugin without a GUI
		$tabParams['event'] = 'onAfterSave'; // onBeforeSave or onAfterSave
		$tabParams['title'] = 'Field set test';
	...
	case 'view':
		...
		$itemArray = array(
			...
			'anchor' => '100%',
			'value' => $modx->visioncart->getProductCustomField($productId, 'testfieldset') // Get a custom field value from the database
		);
	...
	case 'process':
		...
		$modx->visioncart->setProductCustomField($productId, 'testfieldset', $formData['testfieldset']); // Set a custom field value from the database
		break;	
}