...
$currentPictures = $product->get('pictures');

// Get the highest thumbnail number and add one (for unique file naming)
if (empty($currentPictures)) {
	$thumbnailNumber = 1;	
} else {
	$thumbnailNumber = 1;
	foreach($currentPictures as $thumb) {
		$thumb = explode('_', $thumb);
		$thumb = explode('.', $thumb[1]);
		if ($thumb[0] >= $thumbnailNumber) {
			$thumbnailNumber = $thumb[0] + 1;
		}
	}	
}
...
foreach($thumbnailArray as $thumb) {
	...
	imagejpeg($target, $targetDir.$thumb['prefix'].$prodId.'_'.$thumbnailNumber.'.jpg', 90);
	imagedestroy($target);
}
...
$currentPictures[] = $prodId.'_'.$thumbnailNumber.'.jpg';
$product->set('pictures', $currentPictures);
$product->save();
